...
// mlp weights
float **w1, **w11, **w111;    // input layer to 1st hidden layer wts
float **w2, **w22, **w222;    // 1st layer to 2nd layer wts
float **w3, **w33, **w333;    // 2nd layer to 3nd layer wts
float **w4, **w44, **w444;    // 3nd layer to 4nd layer wts
float **w5, **w55, **w555;    // 4nd layer to 5nd layer wts
...
int* generateOrdering(int NumTrnPats);
...
int main() {
  ...
  if (NumIPs < 1 || NumIPs > MAXN || NumOPs < 1 || NumOPs > MAXN
      ...
      || NumHN1 > 50 || LrnRate < 0 || LrnRate > 1 || Mtm1 < 0
      || Mtm1 > 10 || Mtm2 < 0 || Mtm2 > 10 || ObjErr < 0
      || ObjErr > 10) {
...
// x[][]=>input data, d[][]=>desired output data
  ...
  float *h1 = new float[NumHN1]; // O/Ps of hidden layer 1st
  float *h2 = new float[NumHN2]; // O/Ps of hidden layer 2nd
  float *h3 = new float[NumHN3]; // O/Ps of hidden layer 2nd
  float *h4 = new float[NumHN4]; // O/Ps of hidden layer 2nd
  ...
  float *ad4 = new float[NumHN4]; // O/P back prop errors
  ...
  int *shuffleArray = generateOrdering(NumPats);
  cout << "TrainNet4: IP:" << NumIPs << " H1:" << NumHN1 << " H2:" << NumHN2
      << " H3:" << NumHN3 << " H4:" << NumHN4 << " OP:" << NumOPs << endl;
  ...
  w2 = Aloc2DAry(NumHN1, NumHN2); // 1st hidden layer to 2nd layer wts
  w22 = Aloc2DAry(NumHN1, NumHN2);
  w222 = Aloc2DAry(NumHN1, NumHN2);

  w3 = Aloc2DAry(NumHN2, NumHN3); // 2nd hidden layer to 3nd layer wts
  w33 = Aloc2DAry(NumHN2, NumHN3);
  w333 = Aloc2DAry(NumHN2, NumHN3);

  w4 = Aloc2DAry(NumHN3, NumHN4); // 3nd hidden layer  to 4nd layer wts
  w44 = Aloc2DAry(NumHN3, NumHN4);
  w444 = Aloc2DAry(NumHN3, NumHN4);

  w5 = Aloc2DAry(NumHN4, NumOPs); // 4nd hidden layer  to output layer wts
  ...
  for (;;) { // Main learning loop
    ...
    for (p = 0; p < NumPats; p++) { // for each pattern...
      ...
      for (i = 0; i < NumHN1; i++) { // Cal O/P of hidden layer 1
        ...
        for (j = 0; j < NumIPs; j++)
          ...
          case 1:
            // get random index
            in += w1[j][i] * x[shuffleArray[p]][j];
            break;
          case 2:
            if (ItCnt > 0) {
              int j1 = rand() % NumPats;
              int j2 = rand() % NumPats;
              swap(shuffleArray, j1, j2);
            }
            // get random index
            in += w1[j][i] * x[shuffleArray[p]][j];
            break;
      ...
      for (i = 0; i < NumOPs; i++) { // Modify 4nd hidden layer to output layer wts
        ad5[i] = (d[p][i] - y[i]) * y[i] * (1.0 - y[i]);
        for (j = 0; j < NumHN4; j++) {
      ...
      // Learn pattern with back propagation
      for (i = 0; i < NumHN4; i++) { // Modify 3nd hidden layer to 4nd layer wts
        float err = 0.0;
        for (j = 0; j < NumHN3; j++)
          err += ad5[j] * w5[i][j];
        ad4[i] = err * h4[i] * (1.0 - h4[i]);
        for (j = 0; j < NumHN3; j++) {
      ...
      // Learn pattern with back propagation
      for (i = 0; i < NumHN3; i++) { // Modify 2nd hidden layer to layer 3nd hidden layer wts
        float err = 0.0;
        for (j = 0; j < NumHN2; j++)
          err += ad4[j] * w4[i][j];
      ...
      // Learn pattern with back propagation
      for (i = 0; i < NumHN2; i++) { // Modify 1st hidden layer to layer 2nd hidden layer wts
        float err = 0.0;
        for (j = 0; j < NumHN1; j++)
          err += ad3[j] * w3[i][j];
      ...
      for (i = 0; i < NumHN1; i++) { // Modify input layer to 1st hidden layer 1 wts
        float err = 0.0;
        for (j = 0; j < NumIPs; j++)
          err += ad2[j] * w2[i][j];
    ...
    cout << setprecision(6) << setw(6) << ItCnt << ": " << setw(12)
        << MinErr << setw(12) << AveErr << setw(12) << MaxErr
        << setw(12) << PcntErr << endl;

    if ((AveErr <= ObjErr) || (ItCnt == NumIts))
      break;
  } // end main learning loop
  cout << "Train4 - Ordering: " << Ordering << endl;
  // Free memory
  delete[] h1;
  delete[] h2;
  delete[] h3;
...
// x[][]=>input data, d[][]=>desired output data
  ...
  long NumErr = 0; // Error counter (added for spiral problem)
  int *shuffleArray = generateOrdering(NumPats);
  cout << "TrainNet3: IP:" << NumIPs << " H1:" << NumHN1 << " H2:" << NumHN2
  ...
  w4 = Aloc2DAry(NumHN3, NumOPs); // 3nd hidden layer  to output layer wts
  ...
  for (;;) { // Main learning loop
    ...
    for (p = 0; p < NumPats; p++) { // for each pattern...
      ...
      // Learn pattern with back propagation
      for (i = 0; i < NumOPs; i++) { // Modify 3nd hidden layer to output layer wts
  ...
  cout << "Train3 - Ordering: " << Ordering << endl;
  // Free memory
  delete[] h1;
  delete[] h2;
  delete[] h3;
...
// x[][]=>input data, d[][]=>desired output data
  ...
  w3 = Aloc2DAry(NumHN2, NumOPs); // 2nd hidden layer  to output layer wts
  ...
  for (;;) { // Main learning loop
    ...
    for (p = 0; p < NumPats; p++) { // for each pattern...
      ...
      // Learn pattern with back propagation
      for (i = 0; i < NumOPs; i++) { // Modify 2nd hidden layer to output layer wts
  ...
  cout << "Train2 - Ordering: " << Ordering << endl;
  // Free memory
  delete[] h1;
  delete[] h2;
  delete[] y;
...
// x[][]=>input data, d[][]=>desired output data
  ...
  long NumErr = 0; // Error counter (added for spiral problem)
  int *shuffleArray = generateOrdering(NumPats);
  cout << "TrainNet1: IP:" << NumIPs << " H1:" << NumHN1 << " OP:" << NumOPs
      << endl;
  ...
  cout << "Train1 - Ordering: " << Ordering << endl;
  // Free memory
  delete[] h1;
  delete[] y;
  delete[] ad1;
